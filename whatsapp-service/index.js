...
const WEBHOOK_URL = process.env.WEBHOOK_URL || ''; // Sin webhook entrante (solo notificaciones salientes)
const INSTANCE_NAME = process.env.INSTANCE_NAME || 'PREDICTA';
const API_KEY = process.env.API_KEY || ''; // Sin .env el servicio rechaza envíos (guard 401)
...
// ── Predicta: guard de API key para endpoints de escritura ──
function requireApiKey(req, res, next) {
  ...
  if (!key || key !== API_KEY) {
    return res.status(401).json({
      status: 401,
      error: 'Unauthorized',
      response: { message: 'API key inválida o ausente' }
    });
  }
  next();
}
...
app.post('/message/sendText/:instanceName', requireApiKey, async (req, res) => {
  const { instanceName } = req.params;
...
// 6. Cerrar sesión
app.delete('/instance/logout/:instanceName', requireApiKey, async (req, res) => {
  const { instanceName } = req.params;
